...
public class WSProducer3 {
     ...
     */
    public WSProducer3(String addr, int dataPort, String myHostname) {
	// set up data source
	...
	if (args.length == 0) {
	    ...
	    System.err.println("WSProducer3 address port");
	    System.exit(1);
	}
	...
	// we got a hostname
	WSProducer3 producer = new WSProducer3(addr, port, currentHost);

	Probe infra = new InfraProbe(currentHost + ".vee.2.test", currentHost);

	infra.setServiceID(new ID(12345));
	infra.setGroupID(new ID(2));
        infra.setDataRate(new EveryNSeconds(5));

	producer.turnOnProbe(infra);
