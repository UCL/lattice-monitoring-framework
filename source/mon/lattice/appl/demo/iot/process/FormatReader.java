...
// From https://gist.github.com/pgerakios/08bba253a7924c86c87d
// Was:  package vpnrevfwd.util.datastructure;
...
public class FormatReader extends BufferedReader {
	 ...
	 * argument. The character input will be checked against the format string
	 * for
	 * exact matches until an escape sequence is found. Escape sequences, as
	 * usual
	...
	private void parse(int c, TapeReader fmt, List<Object> data) throws IOException, ParseException {
		int f;
		f = fmt.next();
		if (c == -1 || fmt.end())
			return;
	 ...
	 */
	private long getInteger(int c, TapeReader fmt, int base) throws IOException, ParseException {
		// long value = 0;
		// int digit;
		// int c;
	...
	private class TapeReader {
		...
		/**
		 * 
		 * @return current character and advances the head or -1 when end of
		 *         string
		 *         is reached
		 */
		public int next() {
	...
	public static Object[] sscanf(String format, String input) {
		Object[] ret;
		try {
			ret = new FormatReader(new StringReader(input)).scanf(format);
			return ret;
	...
	public static <M> M cast(Object[] obj, int i) { 
		return (M) obj[i];
	}
	...
	public static void main(String[] args) throws Throwable {
		String[][] testPair = { { "this is a test.", "%s is a %s." }, { "line 1" + ls + "line 2", "%s%n%s" },
				{ "command(arg)", "%s(%s)" },
				{ "A test has been completed within 03:24 min", "A test has been completed within %d:%d min" },
				{ "pi = 3.14159", "pi = %f" }, { "testing octal = 2634%", "testing octal = %o%%" },
				{ "testing hex = baadf00d", "testing hex = %x" },
				{ "2009-03-21 13:28:32 [INFO] Trying to log anything", "%d-%d-%d %d:%d:%d [%s] %s" },
		...
		for (int n = 0; n < attempts; ++n) {
			for (int t = 0; t < testPair.length; ++t) {
				...
				Object[] ret = new FormatReader(new StringReader(pair[0])).scanf(pair[1]);
